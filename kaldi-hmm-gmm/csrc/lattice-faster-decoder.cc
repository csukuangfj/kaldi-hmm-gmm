// kaldi-hmm-gmm/csrc/lattice-faster-decoder.cc
...
void LatticeFasterDecoderTpl<FST, Token>::InitDecoding() {
  ...
  KHG_ASSERT(start_state != fst::kNoStateId);
...
bool LatticeFasterDecoderTpl<FST, Token>::GetBestPath(
    fst::Lattice *olat, bool use_final_probs) const {
  fst::Lattice raw_lat;
...
bool LatticeFasterDecoderTpl<FST, Token>::GetRawLattice(
    fst::Lattice *ofst, bool use_final_probs) const {
  typedef fst::LatticeArc Arc;
  ...
  if (decoding_finalized_ && !use_final_probs)
    KHG_ERR << "You cannot call FinalizeDecoding() and then call "
  ...
  for (int32 f = 0; f <= num_frames; f++) {
    if (active_toks_[f].toks == NULL) {
      KHG_WARN << "GetRawLattice: no tokens active on frame " << f
  ...
  for (int32 f = 0; f <= num_frames; f++) {
    for (Token *tok = active_toks_[f].toks; tok != NULL; tok = tok->next) {
      ...
      for (ForwardLinkT *l = tok->links; l != NULL; l = l->next) {
        typename std::unordered_map<Token *, StateId>::const_iterator iter =
            tok_map.find(l->next_tok);
        StateId nextstate = iter->second;
        KHG_ASSERT(iter != tok_map.end());
        float cost_offset = 0.0;
        if (l->ilabel != 0) {  // emitting..
          KHG_ASSERT(f >= 0 && f < cost_offsets_.size());
      ...
      if (f == num_frames) {
        if (use_final_probs && !final_costs.empty()) {
          typename std::unordered_map<Token *, float>::const_iterator iter =
...
LatticeFasterDecoderTpl<FST, Token>::FindOrAddToken(StateId state,
  ...
  KHG_ASSERT(frame_plus_one < active_toks_.size());
...
void LatticeFasterDecoderTpl<FST, Token>::PruneForwardLinks(
  ...
  *links_pruned = false;
  KHG_ASSERT(frame_plus_one >= 0 && frame_plus_one < active_toks_.size());
  if (active_toks_[frame_plus_one].toks ==
      NULL) {  // empty list; should not happen.
    if (!warned_) {
      KHG_WARN << "No tokens alive [doing pruning].. warning first "
  ...
  while (changed) {
    ...
    for (Token *tok = active_toks_[frame_plus_one].toks; tok != NULL;
      ...
      for (link = tok->links; link != NULL;) {
        ...
        KHG_ASSERT(link_extra_cost == link_extra_cost);  // check for NaN
          ...
          *links_pruned = true;
          ...
          if (link_extra_cost < 0.0) {  // this is just a precaution.
            if (link_extra_cost < -0.01)
              KHG_WARN << "Negative extra_cost: " << link_extra_cost;
            link_extra_cost = 0.0;
          }
...
void LatticeFasterDecoderTpl<FST, Token>::PruneForwardLinksFinal() {
  KHG_ASSERT(!active_toks_.empty());
  int32 frame_plus_one = active_toks_.size() - 1;

  if (active_toks_[frame_plus_one].toks ==
      NULL)  // empty list; should not happen.
    KHG_WARN << "No tokens alive at end of file";

  typedef typename std::unordered_map<Token *, float>::const_iterator IterType;
...
template <typename FST, typename Token>
float LatticeFasterDecoderTpl<FST, Token>::FinalRelativeCost() const {
...
void LatticeFasterDecoderTpl<FST, Token>::PruneTokensForFrame(
  ...
  KHG_ASSERT(frame_plus_one >= 0 && frame_plus_one < active_toks_.size());
  Token *&toks = active_toks_[frame_plus_one].toks;
  if (toks == NULL) KHG_WARN << "No tokens alive [doing pruning]";
...
template <typename FST, typename Token>
void LatticeFasterDecoderTpl<FST, Token>::PruneActiveTokens(float delta) {
  ...
  KHG_LOG << "PruneActiveTokens: pruned tokens from " << num_toks_begin
          << " to " << num_toks_;
}
...
void LatticeFasterDecoderTpl<FST, Token>::ComputeFinalCosts(
  ...
  KHG_ASSERT(!decoding_finalized_);
...
void LatticeFasterDecoderTpl<FST, Token>::AdvanceDecoding(
  ...
  KHG_ASSERT(!active_toks_.empty() && !decoding_finalized_ &&
  ...
  // (which isn't allowed).
  KHG_ASSERT(num_frames_ready >= NumFramesDecoded());
...
void LatticeFasterDecoderTpl<FST, Token>::FinalizeDecoding() {
  ...
  KHG_LOG << "pruned tokens from " << num_toks_begin << " to " << num_toks_;
}

/// Gets the weight cutoff.  Also counts the active tokens.
template <typename FST, typename Token>
float LatticeFasterDecoderTpl<FST, Token>::GetCutoff(Elem *list_head,
                                                     size_t *tok_count,
                                                     float *adaptive_beam,
                                                     Elem **best_elem) {
  ...
  if (config_.max_active == std::numeric_limits<int32>::max() &&
    ...
    KHG_LOG << "Number of tokens active on frame " << NumFramesDecoded()
            << " is " << tmp_array_.size();
...
template <typename FST, typename Token>
float LatticeFasterDecoderTpl<FST, Token>::ProcessEmitting(
    DecodableInterface *decodable) {
  KHG_ASSERT(active_toks_.size() > 0);
  int32 frame =
  ...
  KHG_LOG << "Adaptive beam on frame " << NumFramesDecoded() << " is "
          << adaptive_beam;

  PossiblyResizeHash(
      tok_cnt);  // This makes sure the hash is always big enough.

  float next_cutoff = std::numeric_limits<float>::infinity();
  // pruning "online" before having seen all tokens

  float cost_offset = 0.0;  // Used to keep probabilities in a good
                            // dynamic range.
  ...
  if (best_elem) {
    ...
    for (fst::ArcIterator<FST> aiter(*fst_, state); !aiter.Done();
      ...
      if (arc.ilabel != 0) {  // propagate..
        float new_weight = arc.weight.Value() + cost_offset -
...
template <typename FST, typename Token>
void LatticeFasterDecoderTpl<FST, Token>::ProcessNonemitting(float cutoff) {
  KHG_ASSERT(!active_toks_.empty());
  ...
  KHG_ASSERT(queue_.empty());

  if (toks_.GetList() == NULL) {
    if (!warned_) {
      KHG_WARN << "Error, no surviving tokens: frame is " << frame;
      warned_ = true;
    }
  }
  ...
  while (!queue_.empty()) {
    ...
    for (fst::ArcIterator<FST> aiter(*fst_, state); !aiter.Done();
      ...
      if (arc.ilabel == 0) {  // propagate nonemitting only...
        float graph_cost = arc.weight.Value(), tot_cost = cur_cost + graph_cost;
        if (tot_cost < cutoff) {
          bool changed;
...
void LatticeFasterDecoderTpl<FST, Token>::TopSortTokens(
  ...
  for (loop_count = 0; !reprocess.empty() && loop_count < max_loop;
    ...
    for (typename std::unordered_set<Token *>::iterator iter =
             reprocess.begin();
  ...
  KHG_ASSERT(loop_count < max_loop &&
...
template class LatticeFasterDecoderTpl<fst::VectorGrammarFst,
                                       decoder::StdToken>;
#endif
